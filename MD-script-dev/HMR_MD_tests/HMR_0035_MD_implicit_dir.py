...
def find_input_files(input_dir):
    ...
    if not top_files or not crd_files:
        raise FileNotFoundError("Topology or coordinate files not found in the specified directory.")

    # Build a mapping of PDB IDs to files
    tops_by_id = {}
    for top in top_files:
        match = re.search(r"([a-zA-Z0-9]+)\.top$", os.path.basename(top))
        if match:
            pdb_id = match.group(1)
            tops_by_id[pdb_id] = top

    for crd in crd_files:
        match = re.search(r"([a-zA-Z0-9]+)\.crd$", os.path.basename(crd))
        if match:
            pdb_id = match.group(1)
            if pdb_id in tops_by_id:
                return pdb_id, tops_by_id[pdb_id], crd

    raise ValueError("No matching .top and .crd files with the same PDB ID found.")


def create_pdb(top_file, crd_file, output_pdb):
    print(top_file, crd_file, output_pdb)
...
def main():
    ...
    try:
        pdb_id, top_file, crd_file = find_input_files(input_dir)
