...
def process_pdbs(csv_file, output_dir="./divided-files"):
    df = pd.read_csv(csv_file)

    for _, row in df.iterrows():
        pdb_code = row["PDB code"]
        source_path = f"./trunc-cap-cleaned-files/{pdb_code}-8.0-trunc_cap_cleaned.pdb"
...
process_pdbs("refined_gold_data_only_full.csv")
