...
LigandRes=$2
structure=$1
complex_original_pdb=gold_set/${structure}.pdb
...
grep 'ATOM' trunc.pdb | awk '{print substr($0, 23, 4)}' | sort -n | uniq | awk 'NR==1{first=$1;last=$1;next} $1 == last+1 {last=$1;next} {print first,last;first=$1;last=first} END{print first,last}' > resNo.txt
...
# —————————————————————————————
# 3.6+3.7) Rename caps in one pass with AWK
# AceRes and NmeRes are comma-separated lists of ORIGINAL PDB residue numbers
# (built earlier in your script).
# —————————————————————————————

awk -v ace_list="$AceRes" -v nme_list="$NmeRes" '
  BEGIN {
    FS=""; OFS=""
    # build maps for quick lookup
    split(ace_list, a, ",")
    for (i in a) ace_map[a[i]+0] = 1
    split(nme_list, n, ",")
    for (i in n) nme_map[n[i]+0] = 1
  }
  # if it’s not an ATOM record, just print it
  substr($0,1,4) != "ATOM" {
    print; next
  }
  {
    # grab the PDB residue number (cols 23–26), strip spaces, convert to int
    resnum = int( substr($0,23,4) )
    # col 1–17 (up to but not including the 3‐letter resname)
    prefix = substr($0,1,17)
    # cols 18–20 are the 3‐letter name, cols 21–end are everything else
    rest   = substr($0,21)
    if (resnum in ace_map) {
      name = "ACE"
    } else if (resnum in nme_map) {
      name = "NME"
    } else {
      name = substr($0,18,3)
    }
...
' trunc_cap.pdb > tmp.pdb && mv tmp.pdb trunc_cap.pdb


# grep '^ATOM' trunc.pdb | awk '{print substr($0, 23, 4)}' | sort -u | wc -l
# echo $structure
cat trunc_cap.pdb > "truncated_gold/$structure-$Radius.pdb"
