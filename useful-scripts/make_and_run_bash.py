...
def main(input_dir, csv_file, gpu_id):
    ...
    for pdb_code in df["PDB codes"].dropna().astype(str):
        ...
        script_path = create_bash_script(pdb_code, gpu_id, subdir_path)
        print(f"Running script for {pdb_code}...")
        result = subprocess.run([script_path], cwd=subdir_path)
        if result.returncode != 0:
            print(f"Error: Script failed for {pdb_code} with return code {result.returncode}")
        else:
